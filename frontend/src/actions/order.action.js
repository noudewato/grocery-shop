...
export const orderListMyAction = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get('/api/order/get-user-order', config);
...
export const orderStatusAction = () => async (dispatch, getState) => {
  try {
    dispatch({
    ...
    const { data } = await axios.get(`/api/order/status`,  config);
